...
class Evaluate():
    ...
    # Individual metric methods for easier access
    def accuracy(self):
        _, acc, _, _, _ = self.confusion_matrix()
        return acc
    
    def specificity(self):
        _, _, sp, _, _ = self.confusion_matrix()
        return sp
    
    def sensitivity(self):
        _, _, _, se, _ = self.confusion_matrix()
        return se
    
    def precision(self):
        _, _, _, _, pr = self.confusion_matrix()
        return pr
...
def compute_metrics(preds, targets):
    ...
    Args:
        preds: Predicted masks (numpy array, binary or probabilities)
        targets: Ground truth masks (numpy array, binary)
    ...
    if preds_flat.max() <= 1.0 and preds_flat.min() >= 0.0 and preds_flat.dtype != bool:
        preds_binary = preds_flat >= 0.5
    else:
        preds_binary = preds_flat
    
    # Compute confusion matrix metrics
    confusion = confusion_matrix(targets_flat, preds_binary)
    ...
    # Calculate metrics if confusion matrix is valid
    if confusion.size == 4:
    ...
    # Compute AUC-ROC
    try:
        auc_roc = roc_auc_score(targets_flat, preds_flat)
    except:
        auc_roc = 0.5
