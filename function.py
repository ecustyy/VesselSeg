...
def get_dataloaderV2(args):
    ...
    if args.sample_visualization:
        ...
        N_sample = 50
        visual_imgs = np.empty((N_sample,3,args.train_patch_height, args.train_patch_width))
        visual_masks = np.empty((N_sample,3,args.train_patch_height, args.train_patch_width))

        for i, (img, mask) in tqdm(enumerate(visual_loader)):
            visual_imgs[i] = img.numpy()
            visual_masks[i, 0] = mask.numpy()
            if i>=N_sample-1:
                break
